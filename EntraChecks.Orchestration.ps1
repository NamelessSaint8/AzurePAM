<#
.SYNOPSIS
    EntraChecks.Orchestration.ps1 - dot-sourceable orchestration include.

.DESCRIPTION
    Native App Plan, Phase 1 (4/4). The function definitions formerly
    inline in Start-EntraChecks.ps1 (display, authentication, module
    execution, interactive mode, Quick/Scheduled/Hybrid modes, and the
    Invoke-EcfAssessmentSequence runner seam). Pure definitions only -
    no param block, no setup, no dispatch.

    Dot-sourced (not imported) so every $script: reference binds to the
    caller's scope exactly as when these were inline:
      - Start-EntraChecks.ps1 dot-sources this after its setup block,
        so the menu/modes see $script:Config / ModulesPath / Findings
        identically to before (behaviour-identical refactor).
      - Invoke-EntraChecksRun.ps1 (the headless wrapper / Phase 4 Tauri
        sidecar entrypoint) dot-sources the same file after an
        equivalent setup, giving the runner the orchestration without
        relocating the cross-module $script: substrate.

    $PSScriptRoot inside these functions resolves to this file's
    directory (the repo root) - identical to Start-EntraChecks.ps1's,
    so the config/Scripts path joins are unchanged.

.NOTES
    Mechanical extraction of Start-EntraChecks.ps1 function regions.
    No logic changes. See
